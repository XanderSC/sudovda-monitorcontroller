...
```
SudoVDA-MonitorController\
    SudoVDA-MonitorController.log      <- runtime log (root, regardless of layout changes)
    Common\
        RunMonitorController.bat        <- launcher used by the scheduled task
        CreateScheduledTask.ps1         <- one-time setup script
    Controller\
        SudoVDA-MonitorController.ps1   <- main script
        MultiMonitorTool.exe            <- you provide this, see Setup below
    Autogenerated\
        MonitorConfig-Normal.cfg        <- your saved monitor layout
        MultiMonitorTool.sha256         <- integrity hash for the bundled tool
        monitors-temp.txt               <- transient, used and deleted each loop
```

Every path is resolved at runtime relative to the script's own location (`$PSScriptRoot` / `%~dp0`), so this whole structure can be moved or renamed at the top level and everything still works without editing any file.

## Setup

1. Clone or download this repo to a folder of your choice, e.g. `C:\Tools\SudoVDA-MonitorController\`.
2. Download `MultiMonitorTool.exe` from [nirsoft.net](https://www.nirsoft.net/utils/multi_monitor_tool.html) and place it in the **`Controller`** folder, next to `SudoVDA-MonitorController.ps1`.
3. Open PowerShell **as Administrator** and run the setup script from the **`Common`** folder:
   ```powershell
   & "C:\Tools\SudoVDA-MonitorController\Common\CreateScheduledTask.ps1"
   ```
4. Done. The task now runs automatically at every login. The first time it runs it will:
   - Create the `Autogenerated` folder if it doesn't already exist
...
```powershell
& "C:\Tools\SudoVDA-MonitorController\Common\RunMonitorController.bat" 2>&1
```
...
```powershell
& "C:\Tools\SudoVDA-MonitorController\Controller\MultiMonitorTool.exe" /LoadConfig "C:\Tools\SudoVDA-MonitorController\Autogenerated\MonitorConfig-Normal.cfg"
```
...
- **Changed your physical monitor setup permanently** (added/removed a monitor, repositioned them)? Just re-save the baseline config:
  ```powershell
  & "C:\Tools\SudoVDA-MonitorController\Controller\MultiMonitorTool.exe" /SaveConfig "C:\Tools\SudoVDA-MonitorController\Autogenerated\MonitorConfig-Normal.cfg"
  ```
- **Updated MultiMonitorTool.exe**? Delete the hash file so it's regenerated on the next run:
  ```powershell
  Remove-Item "C:\Tools\SudoVDA-MonitorController\Autogenerated\MultiMonitorTool.sha256" -Force
  ```
...
| File | Purpose |
|---|---|
| `Controller\SudoVDA-MonitorController.ps1` | Main script — all detection and control logic |
| `Controller\MultiMonitorTool.exe` | NirSoft dependency you provide — see Setup |
| `Common\RunMonitorController.bat` | Launcher used by the scheduled task |
| `Common\CreateScheduledTask.ps1` | One-time setup script to register the scheduled task |
| `Autogenerated\MonitorConfig-Normal.cfg` | Auto-generated on first run — your saved monitor layout |
| `Autogenerated\MultiMonitorTool.sha256` | Auto-generated on first run — integrity hash for the bundled tool |
| `SudoVDA-MonitorController.log` | Runtime log (root folder), useful for troubleshooting |
